...
{
    private readonly string connectionString = "Data Source = DB/databaseuwu.db; Cache = Shared";
    public Tarea CreateTask(int boardId, Tarea task) {
        ...
        command.Parameters.Add(new SQLiteParameter("@id_tablero", task.IdTablero));
    ...
    public List<Tarea> GetTasksByUser(int userId) {
        ...
        {
            var command = new SQLiteCommand(query, connection);
            command.Parameters.Add(new SQLiteParameter("@idUser", userId));
            connection.Open();
            using (SQLiteDataReader reader = command.ExecuteReader())
            {
